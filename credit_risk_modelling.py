...
Original file is located at
    https://colab.research.google.com/drive/15aWPCSlFe0X92oEbhsj1be2ykyaQUdl8
"""
...
# removing columns having null values as this columns are less
...
## we try to implement ML models to check performance and then select the better one.
...
#above XGBoost perform best among decision tree and random forest then we try to tune it for best performance from above 

# 2. xgboost hyper parameter optimized
...
#storing the optimized model using pickle
import pickle
filename = "CRM_model.pkl"
pickle.dump(xgb_classifier,open(filename, 'wb'))

new_df = df_encoded.drop(['Approved_Flag'], axis=1)
new_df = pd.concat([new_df, df_unseen_encoded])
new_df.to_csv('final_data.csv', index=False)

# # Convert the dictionary to a DataFrame
# CRM_hyper = pd.DataFrame(answers_grid)

# # Specify the CSV file name
# csv_file = "output.csv"

# # Save the DataFrame to a CSV file
# CRM_hyper.to_csv(csv_file, index=False)

#loading the unseen data by model and try to predict fot it and analyze the distribution of categories
...
# Applying LIME to understand prediction for particular instance 
import lime
...
instance = x_test.iloc[43].values
...
# Show the explanation in notebook (tabular format)
...
plt.savefig('lime_explanation.png')  # Save as PNG file
